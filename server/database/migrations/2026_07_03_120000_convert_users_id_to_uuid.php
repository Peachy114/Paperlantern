...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // NOTE: users.id -> UUID already done. withdrawal_requests and
        // sticky_notes columns already swapped to UUID. No FKs re-added
        // yet anywhere (the re-add loop never ran). Resuming from
        // chapter_likes onward.
        ...
        foreach ($this->simpleForeignKeys as $fk) {
            $old = $fk['column'];
            $new = "{$fk['column']}_uuid";
            DB::statement("ALTER TABLE `{$fk['table']}` DROP COLUMN `{$old}`");
            DB::statement("ALTER TABLE `{$fk['table']}` CHANGE `{$new}` `{$old}` CHAR(36) NULL");
        }
