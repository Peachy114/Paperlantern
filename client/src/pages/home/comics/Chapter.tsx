...
export default function ComicChapter() {
  const {
    chapter, prevId, nextId, nextChapter, prevChapter,
    ...
    <div className="flex items-center justify-between py-2.5 gap-2">
      ...
      {prevId ? (
        prevChapter?.is_locked ? (
          ...
          >
            🔒 <span className="hidden xs:inline">{prevChapter.credits_required} CREDITS TO </span>UNLOCK
          </button>
        ) : (
          <button
            onClick={() => goTo(prevId)}
            ...
            style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(11px, 3vw, 14px)', letterSpacing: '0.15em', boxShadow: '2px 2px 0 var(--foreground)' }}
          >
            ← PREV
          </button>
        )
      ...
      {nextId ? (
        nextChapter?.is_locked ? (
          <button
            ...
            style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(11px, 3vw, 14px)', letterSpacing: '0.1em', boxShadow: '2px 2px 0 var(--foreground)' }}
          >
            🔒 <span className="hidden xs:inline">{nextChapter.credits_required} CREDITS TO </span>UNLOCK
          </button>
        ) : (
          <button
            onClick={() => goTo(nextId)}
      ...
      <div className="max-w-3xl mx-auto px-3 sm:px-4 py-5 sm:py-8" ref={contentRef}>
        ...
        <div className="flex items-center justify-between pb-3 sm:pb-4 mb-4 sm:mb-5 border-b-[2.5px] border-[#1a1a1a] dark:border-foreground/40 gap-2">
          <button
            ...
            style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '11px', letterSpacing: '0.2em' }}
          >
            ← BACK
          </button>
          ...
          <div className="flex-1 min-w-0 text-center">
            <span
              ...
              style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(9px, 2.5vw, 11px)', letterSpacing: '0.2em' }}
            >
              ◆ {chapter.work_type === 'webtoon' ? 'WEBTOON' : 'NOVEL'} · CH.{chapter.order}
            </span>
            <h1
              className="text-foreground leading-none truncate"
              style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(16px, 4vw, 32px)', letterSpacing: '0.04em' }}
            >
              {chapter.title}
            </h1>
          </div>
        ...
        <div className="flex flex-col items-center gap-1.5 py-4 sm:py-5">
          <button
            ...
            style={{ fontFamily: "'Kalam', cursive", fontSize: 'clamp(12px, 3vw, 13px)' }}
          >
            <span className={`text-lg transition-colors duration-150 ${liked ? 'text-red-400' : 'text-muted-foreground group-hover:text-red-400'}`}>
              {liked ? '♥' : '♡'}
            </span>
            <span className={`transition-colors duration-150 ${liked ? 'text-muted-foreground/50' : 'text-muted-foreground group-hover:text-muted-foreground/50'}`}>
              {liked ? 'thank you for the love!' : (
                <>
                  <span className="hidden xs:inline">if you enjoyed this, leave a like</span>
          ...
          <span
            ...
            style={{ fontFamily: "'Kalam', cursive", fontSize: 'clamp(11px, 3vw, 13px)' }}
          >
