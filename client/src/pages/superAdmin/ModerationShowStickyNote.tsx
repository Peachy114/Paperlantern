...
import { useAdminModerationShowStickyNote } from '@/hooks/useAdminModerationShowStickyNote'
import { storageUrl } from '@/utils/storage'

const FONTS = "https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Kalam:wght@400;700&display=swap"
      ...
      <div className="max-w-4xl mx-auto px-3 sm:px-4 py-6 sm:py-10">
        ...
        <div className="flex gap-0">
          ...
          <div className="flex-1 border-[2.5px] border-[#1a1a1a] bg-[#fffdf5] dark:bg-[#1c1a17]" style={{ boxShadow: '4px 4px 0 #1a1a1a' }}>
            ...
            style={{ background: note.color ?? '#fef9c3', fontFamily: "'Kalam', cursive", fontSize: '14px', lineHeight: '1.6', boxShadow: '3px 3px 0 #1a1a1a20' }}
            >
            {note.type === 'text' && (
                note.text
                ? note.text
            ...
            {note.type === 'image' && (
                note.image_path
                ? <img src={storageUrl(note.image_path)!} alt="Sticky note image" className="w-full" />
            ...
            {note.moderation_status !== 'pending_review' && (
              <div
                ...
                style={{ background: note.moderation_status === 'approved' ? 'rgba(134,239,172,0.2)' : 'rgba(252,165,165,0.2)' }}
              >
                <span
                  className="text-[13px]"
                  style={{ fontFamily: "'Kalam', cursive", color: note.moderation_status === 'approved' ? '#16a34a' : '#dc2626' }}
                >
                  {note.moderation_status === 'approved' ? '✓ This note has been approved.' : '✕ This note was marked as a violation.'}
                </span>
              </div>
            )}
