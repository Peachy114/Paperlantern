...
import { useChapterShow } from '@/hooks/useChapterShow'
    ...
    <div className="flex items-center justify-between py-2.5">
      {prevId ? (
        <button
          ...
          style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '14px', letterSpacing: '0.15em', boxShadow: '2px 2px 0 var(--foreground)' }}
        >
          ← PREV
        </button>
      ) : <div />}
      ...
      <div className="max-w-3xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
        ...
        <div className="flex items-start sm:items-center justify-between pb-4 mb-5 border-b-[2.5px] border-[#1a1a1a] dark:border-foreground/40 gap-3">
          ...
          <button
            ...
            style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '12px', letterSpacing: '0.2em' }}
          >
            ← BACK
          </button>

          <div className="min-w-0 flex-1">
            <span className="text-center text-amber-500 block" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '11px', letterSpacing: '0.25em' }}>
              ◆ {chapter.work_type === 'webtoon' ? 'WEBTOON' : 'NOVEL'} · CH.{chapter.order}
            </span>
            <h1 className="text-foreground leading-tight sm:leading-none" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(20px, 5vw, 32px)', letterSpacing: '0.04em', wordBreak: 'break-word' }}>
              {chapter.title}
            </h1>
          </div>

          {/* Date — hidden on very small screens, shown from sm up */}
          <span className="text-muted-foreground/40 shrink-0" style={{ fontFamily: "'Kalam', cursive", fontSize: '11px' }}>
          ...
          <div className="flex flex-col overflow-hidden bg-zinc-950 my-5">
            {chapter.images.length === 0 ? (
              ...
              chapter.images.map((img) => (
                <img key={img.id} src={imageUrl(img.path)!} alt={`Page ${img.order + 1}`} className="w-full block" draggable={false} />
              ))
            )}
          </div>
        )}
        ...
        {chapter.work_type === 'wattpad' && (
          <div className="bg-[#ffffff] dark:bg-[#080808] px-4 sm:px-8 py-8 sm:py-10 my-5 text-start">
            {chapter.content ? (
              <p className="leading-loose text-foreground whitespace-pre-wrap"
                style={{ fontFamily: "'Noto Serif', serif", fontSize: 'clamp(14px, 3.5vw, 15px)' }}>
                {chapter.content}
              </p>
            ) : (
        ...
        <div className="flex items-center justify-between mt-3 px-1">
          <span className="text-muted-foreground/40 text-[10px] tracking-[0.2em]" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
