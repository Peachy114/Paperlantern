// pages/studio/Chapter/Create.tsx
...
function ChapterForm({ workType }: { workType: 'webtoon' | 'wattpad' }) {
  ...
  const [orderedPreviews, setOrderedPreviews] = useState<{ src: string; originalIndex: number }[]>([])
  ...
  const handleDragOver = (e: React.DragEvent, i: number) => {
    ...
    reorderImages(from, to) // 👈 sync hook
        ...
        {workType === 'webtoon' && (
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              ...
              {orderedPreviews.length > 0 && (
                <span className="text-xs text-muted-foreground">
                  {orderedPreviews.length} page{orderedPreviews.length !== 1 ? 's' : ''} · drag to reorder
                </span>
              )}
            </div>
            ...
            <label className="flex items-center justify-center gap-2 h-11 rounded-xl border-2 border-dashed border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 cursor-pointer hover:border-zinc-400 dark:hover:border-zinc-600 transition-colors text-sm text-muted-foreground hover:text-foreground">
              <span>+</span>
              <span>{orderedPreviews.length > 0 ? 'Replace pages' : 'Add pages'}</span>
          ...
          <div className="flex flex-col gap-2 text-start">
            {[
              ...
              return (
                <div
                  key={option.value}
                  onClick={() => handleChange({ target: { name: 'lock_type', type: 'text', value: option.value } } as any)}
