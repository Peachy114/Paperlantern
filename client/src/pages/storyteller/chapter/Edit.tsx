...
export default function ChapterEdit() {
  ...
  useEffect(() => {
    studioApi.getWork(Number(workId)).then((res) => {
      setWorkType(res.data.type)
    }).finally(() => setFetchingWork(false))
  }, [workId])

  if (fetchingWork) return (
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <input
              name="title" value={form.title} onChange={handleChange} required placeholder="Chapter title"
        ...
        {form.status === 'scheduled' && (
          <div className="flex flex-col gap-1.5">
            ...
            <input
              type="datetime-local" name="scheduled_at" value={form.scheduled_at} onChange={handleChange}
              className="h-10 px-3 text-sm rounded-lg bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-foreground focus:outline-none focus:ring-1 focus:ring-zinc-400 transition-all"
            />
          </div>
        )}
        ...
        {workType === 'webtoon' && (
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium text-foreground">Chapter pages</label>
              {imageItems.length > 0 && (
                <span className="text-xs text-muted-foreground">
                  {imageItems.length} page{imageItems.length !== 1 ? 's' : ''} · drag to reorder
                </span>
              )}
            </div>
            ...
            {imageItems.length > 0 && (
              <div className="grid grid-cols-3 gap-2 mt-1">
                {imageItems.map((item, i) => (
                  ...
                  >
                    <img src={item.preview} alt={`Page ${i + 1}`} className="w-full h-full object-cover pointer-events-none" />
        ...
        {workType === 'wattpad' && (
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              name="content" value={form.content} onChange={handleChange} rows={20}
        ...
        <div className="flex flex-col gap-3">
          <p className="text-sm font-medium text-foreground" style={{ fontFamily: "'Kalam', cursive" }}>
            🔒 Chapter access type
          </p>
            ...
            {[
              ...
              return (
                <div
                  key={option.value}
                  onClick={() => {
                    handleChange({ target: { name: 'lock_type', type: 'text', value: option.value } } as any)
                    if ((option.value === 'early_access' || option.value === 'premium') && (!form.credits_required || form.credits_required < 3)) {
                      handleChange({ target: { name: 'credits_required', type: 'number', value: '3' } } as any)
                    }
                  }}
                  ...
                  style={{ boxShadow: active ? `3px 3px 0 ${option.color}` : 'none' }}
                >
                  <div
                    ...
                    style={{ background: active ? option.color : 'transparent' }}
                  >
                    {active && (
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground" style={{ fontFamily: "'Kalam', cursive" }}>
                      {option.desc}
                    </p>
                  </div>

                  {/* Sticky note for active */}
                  {active && (
                    <div
          ...
          {(form.lock_type === 'early_access' || form.lock_type === 'premium') && (
            <div className="flex items-center gap-3 pl-2 pt-1">
              ...
              <input
                ...
                style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '16px', boxShadow: '2px 2px 0 var(--foreground)' }}
              />
              <span className="text-xs text-muted-foreground" style={{ fontFamily: "'Kalam', cursive" }}>
        ...
        <div className="flex items-center justify-end gap-2 pt-2">
          <button
            type="button" onClick={() => navigate(`/studio/works/${workId}/chapters`)}
