    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{
            backgroundImage: 'radial-gradient(circle, rgba(245,158,11,0.12) 1.2px, transparent 1.2px)',
            backgroundSize: '9px 9px',
          }}
        />

        {/* Speed lines top-right */}
        <svg className="absolute top-0 right-0 w-44 h-36 pointer-events-none opacity-[0.06]" viewBox="0 0 180 160">
          {Array.from({ length: 12 }, (_, i) => {
            const rad = ((i / 12) * 90 + 270) * (Math.PI / 180)
            return <line key={i} x1="180" y1="0" x2={180 + Math.cos(rad) * 220} y2={Math.sin(rad) * 220} stroke="#f59e0b" strokeWidth={i % 3 === 0 ? 2 : 1} />
          })}
        </svg>

        {/* Speed lines bottom-left */}
        <svg className="absolute bottom-0 left-0 w-36 h-28 pointer-events-none opacity-[0.06]" viewBox="0 0 140 120">
          {Array.from({ length: 10 }, (_, i) => {
            const rad = ((i / 10) * 90 + 90) * (Math.PI / 180)
            return <line key={i} x1="0" y1="120" x2={Math.cos(rad) * 200} y2={120 + Math.sin(rad) * 200} stroke="#14b8a6" strokeWidth={i % 3 === 0 ? 2 : 1} />
          })}
        </svg>
        ...
        <div className="relative z-10 max-w-[1126px] mx-auto px-5 sm:px-8 md:px-10 pt-8 sm:pt-10 pb-6 sm:pb-7">
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-8 sm:gap-10 items-start">
            ...
            <div className="flex flex-col gap-3 min-w-0 w-full sm:w-auto sm:min-w-[200px]">
              ...
              >
                <span className="text-[22px] leading-none tracking-[0.06em] text-[#1a1a1a]" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
                  PAPER LANTERN
                </span>
              </div>

              <p className=" text-start text-[11px] tracking-[0.1em] text-muted-foreground leading-relaxed max-w-[240px] sm:max-w-[210px]" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
                A COZY CORNER FOR COMICS & NOVELS.{' '}
                READ, DISCOVER, AND SUPPORT CREATORS.
              </p>
                  ...
                  {links.map(({ label, to }) => (
                    !token && PROTECTED.includes(to) ? (
                      <button
                        ...
                        style={{ fontFamily: "'Bebas Neue', sans-serif" }}
                      >
                        <span className="text-[10px] text-amber-500 opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-100 hidden sm:inline">▸</span>
                        {label.toUpperCase()}
                      </button>
                    ) : (
                      <Link
          ...
          <div className="relative mt-8 pt-5 border-t-[2.5px] border-foreground flex flex-col sm:flex-row items-center sm:justify-between gap-3 text-center sm:text-left">
            ...
            <p className="text-[11px] tracking-[0.1em] text-muted-foreground" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
              © {new Date().getFullYear()} PAPER LANTERN. ALL RIGHTS RESERVED.
            </p>
