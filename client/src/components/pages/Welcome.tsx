  ...
  return (
    <>
      ...
      <div className="relative min-h-[82vh] flex flex-col items-center justify-center overflow-hidden bg-[#fffdf5] dark:bg-[#1a1712] px-4 py-10">
        ...
        {/* Halftone bg */}
        <div className="absolute inset-0 pointer-events-none dark:hidden" style={{ backgroundImage: 'radial-gradient(circle, rgba(0,0,0,0.065) 1.2px, transparent 1.2px)', backgroundSize: '9px 9px' }} />
        <div className="absolute inset-0 pointer-events-none hidden dark:block" style={{ backgroundImage: 'radial-gradient(circle, rgba(232,168,56,0.11) 1.2px, transparent 1.2px)', backgroundSize: '9px 9px' }} />
        ...
        >
          ...
          <div className="px-6 sm:px-8 pt-8 pb-7 text-center flex flex-col items-center gap-6">
            ...
            <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.5, delay: 0.05 }} className="flex items-center gap-3 w-full justify-center">
              <div className="flex-1 max-w-[80px] h-[2px] bg-foreground/15" />
              <span className="text-[11px] tracking-[0.24em] text-amber-500" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>✦ A COZY CORNER FOR STORIES ✦</span>
              <div className="flex-1 max-w-[80px] h-[2px] bg-foreground/15" />
            </motion.div>

            <div className="flex items-center justify-center gap-4 sm:gap-5 flex-wrap">
              <motion.svg
                width="80" height="80" viewBox="0 0 80 80" className="flex-shrink-0"
            ...
            <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.5, delay: 0.3 }} className="relative mt-1">
              ...
              >
                <p className="text-[13px] sm:text-[14px] tracking-[0.04em] text-foreground dark:text-[#e8dfc8] leading-relaxed">
                  ...
                  <span className="text-amber-600 dark:text-amber-400">Webcomics, novels &amp; serialized fiction await.</span>
                </p>
              </div>
            </motion.div>

            <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.5, delay: 0.4 }} className="flex flex-wrap items-center justify-center gap-3">
              <motion.a
                href="/all-comics"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 w-full text-left mt-1">
              {([
                { icon: 'ti-book-2', label: 'COMICS', sub: 'webtoons & manga', accent: '#f77c9b', n: '01', dark: false },
                { icon: 'ti-feather', label: 'NOVELS', sub: 'wattpad-style', accent: '#e8a838', n: '02', dark: false },
                { icon: 'ti-bolt', label: 'SUPPORT', sub: 'unlock chapters', accent: '#e8a838', n: '03', dark: true },
              ] as { icon: string; label: string; sub: string; accent: string; n: string; dark: boolean }[]).map(({ icon, label, sub, accent, n, dark }, i) => (
                <motion.div
                  key={label}
                  initial="hidden"
                  animate="visible"
                ...
                >
                  ...
                  <i className={`ti ${icon}`} aria-hidden="true" style={{ fontSize: '26px', color: accent, display: 'block', marginBottom: '10px' }} />
                  <div className={`text-[14px] tracking-[0.1em] font-medium mb-1 ${dark ? 'text-[#fffdf5]' : 'text-foreground'}`}>{label}</div>
                  <div className={`text-[11px] ${dark ? 'text-[#fffdf5]/55' : 'text-foreground/55'}`} style={{ fontFamily: "'Noto Serif', serif" }}>{sub}</div>
                  <div className="h-1 mt-3 w-3/5" style={{ background: accent, transform: 'skewX(-8deg)' }} />
                </motion.div>
              ))}
            </div>

            <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.5, delay: 0.8 }} className="flex items-center gap-2 w-full">
              <div className="flex-1 h-[2px] bg-foreground/10" />
              <span className="text-[10px] tracking-[0.22em] text-muted-foreground" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>★ PAPER LANTERN PUBLISHING ★</span>
              <div className="flex-1 h-[2px] bg-foreground/10" />
            </motion.div>
          </div>

          <div className="absolute bottom-0 left-0 right-0 h-[3px]" style={{ background: 'linear-gradient(90deg, #b45309 0%, #d97706 50%, #e8a838 100%)' }} />
        </div>
      </div>
    </>
  )
