...
import type { Wallet, CreditPackage, WalletTransaction, CheckoutResponse, UnlockResponse } from '@/types/wallet';
...
export function useCreditPackages() {
  ...
  const { data: packages = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<{ packages: CreditPackage[] } | CreditPackage[]>('/credits/packages');
...
export function useWalletTransactions(perPage = 15) {
  ...
  const { data: transactions = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<{ data: WalletTransaction[] }>(`/wallet/transactions?per_page=${perPage}`);
      return data.data;
    },
    enabled: !!token,
    staleTime: 1000 * 30,
...
export async function initiateCheckout(packageId: number): Promise<CheckoutResponse> {
  const { data } = await api.post<CheckoutResponse>('/credits/checkout', { package_id: packageId });
  return data;
}
