...
export default function HeroSection({
    ...
    const slides: Slide[] = React.useMemo(() => {
        const withImage = announcements.filter((a) => !!storageUrl(a.image ?? null))
        const pinnedNews = withImage
            .filter((a) => a.is_pinned)
            .map((a) => ({ kind: 'news' as const, data: a }))
        const unpinnedNews = withImage
            .filter((a) => !a.is_pinned)
            .map((a) => ({ kind: 'news' as const, data: a }))
...
function ComicSlide({
    ...
    return (
        ...
        >
            {/* Dimmed layer: photo + contrast overlay only */}
            <div
            ...
            {/* Bottom fade into page background — kept outside the dim filter so it's consistent on every card */}
            ...
            >
                ...
                <h1
                    className="text-white leading-[0.95] mb-2 line-clamp-2"
                    style={{
    ...
    return (
        ...
        >
            ...
            <div className="absolute top-0 left-0 right-0 px-4 sm:px-6 py-2.5 flex items-center justify-between bg-black/40 backdrop-blur-sm">
                ...
                <span
                    className="text-white/60 text-[9px] tracking-[0.15em]"
                    style={{ fontFamily: "'Bebas Neue', sans-serif" }}
                >
