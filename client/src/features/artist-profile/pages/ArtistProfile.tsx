...
type CanvasItemPatch = Partial<
  Pick<ProfileCanvasItem, "display" | "pagination" | "page">
>;
...
const PROFILE_TAB_IDS: ProfileTabId[] = [
  "board",
  "arts",
  "works",
  "stickers",
...
export default function ArtistProfile() {
  ...
  useEffect(() => {
    ...
    const updateEditorMode = () =>
      setIsDesktopProfileEditor(mediaQuery.matches);
  ...
  const artImages = useMemo(
    () => getArtImageOptions(profile.arts),
    [profile.arts],
  );
  const selectedBlock =
  ...
  const renderableTabs = getRenderableProfileTabs(
    visibleTabs,
    themeDraft.tabsConfig,
  );
  const defaultTab = renderableTabs[0] ?? "board";
  const activeTab = visibleTabs.includes(activeProfileTab)
    ? activeProfileTab
    : defaultTab;
  const showArtsWithBoard =
    isStorytellerProfile &&
    visibleTabs.includes("arts") &&
    ["board_arts", "board_arts_stickers"].includes(
      themeDraft.tabsConfig.section_mode,
    );
  const showStickersWithBoard =
    visibleTabs.includes("stickers") &&
    ["board_stickers", "board_arts_stickers"].includes(
      themeDraft.tabsConfig.section_mode,
    );
  ...
  const saveHeaderFile = async (
    ...
    try {
      await updateHeader.mutateAsync(payload);
      toast.success(
        key === "background_image" ? "Background updated." : "Image updated.",
      );
    } catch {
  ...
  const saveProfileTheme = async () => {
    ...
    try {
      await updateHeader.mutateAsync(
        profileThemeToFormData(themeDraft, headerDraft),
      );
  ...
  const handleNavMove = (event: globalThis.PointerEvent) => {
    ...
    const dxPercent =
      ((event.clientX - drag.startX) / Math.max(canvasWidth, 1)) * 100;
  ...
  const handleTabMove = (event: globalThis.PointerEvent) => {
    ...
    const nextPosition =
      drag.kind === "move"
        ? {
            ...current,
            x: clamp(current.x + dx, 0, 100 - current.w),
    ...
    const nextConfig = patchTabPosition(
      themeDraft.tabsConfig,
      drag.tab,
      nextPosition,
    );
    ...
    const item =
      drag.kind === "move"
        ? {
            ...drag.item,
            x: snapCanvasX(
              clamp(drag.item.x + dx, 0, 100 - drag.item.w),
              drag.item.w,
            ),
  ...
  const addCanvasItemToDraft = (
    ...
    setThemeDraft((current) => {
      ...
      const currentItems = getCanvasItems(
        current.tabsConfig,
        PROFILE_TAB_IDS,
        kind,
      );
  ...
  const removeCanvasItemFromDraft = (
    itemId: string,
    kind: ProfileCanvasItem["kind"],
  ) => {
    setThemeDraft((current) => {
  ...
  const handleCanvasDrop = (event: DragEvent<HTMLDivElement>) => {
    ...
    const payload = parseCanvasDropPayload(
      event.dataTransfer.getData(PROFILE_CANVAS_DROP_MIME),
    );
    const canvas = canvasRef.current;
    ...
    const rect = canvas.getBoundingClientRect();
    const existingItems = getCanvasItems(
      themeDraft.tabsConfig,
      PROFILE_TAB_IDS,
      payload.kind,
    );
    ...
    const height =
      existingItem?.h ??
      (payload.kind === "tab" ? 40 : payload.type === "board" ? 520 : 420);
    const x = snapCanvasX(
      clamp(
    ...
    const y = snapCanvasY(
      clamp(event.clientY - rect.top - height / 2, 0, 2400),
    );
  ...
  const createStickerBoardBlock = async (
    ...
    const y = snapMin(
      (clientY - rect.top) / BOARD_UNIT_PX - STICKER_BLOCK_SIZE.h / 2,
      0,
    );
    const payload = new FormData();
  ...
  const handleStickerDragUp = (event: globalThis.PointerEvent) => {
    ...
    if (sticker)
      void createStickerBoardBlock(sticker, event.clientX, event.clientY);
  };
        ...
        {isOwner && manageProfileMode && (
          <ManageProfileSidebar
            ...
            onUploadBackground={(file) =>
              saveHeaderFile("background_image", file)
            }
...
function ProfileTabsNav({
  ...
  const height = isSeparate
    ? getTabsCanvasHeight(theme.tabsConfig, tabIds)
    : theme.navH;
  ...
  return (
    ...
    >
      ...
      <TabsList
        className={
          isSeparate
            ? "relative block w-full bg-transparent p-0"
      ...
      {item.type === "arts" && (
        <>
          <SelectField
            ...
            options={["masonry", "pinterest", "instagram"]}
...
function ProfileLayoutCanvas({
  ...
  const canvasHeight = Math.max(
    ...
    theme.navLayout === "together"
      ? Math.max(0, 16 + theme.navY + theme.navH + 80)
      : 0,
  );
  ...
  return (
    <div
      ...
      onDragOver={(event) => {
        if (!editMode) return;
        if (!event.dataTransfer.types.includes(PROFILE_CANVAS_DROP_MIME))
          return;
        event.preventDefault();
      ...
      {sections.map((item) => (
        ...
        >
          ...
          <div
            ...
            style={{
              minHeight:
                item.type === "board" ? "calc(100% - 56px)" : undefined,
            }}
          >
            {renderSection(item)}
          </div>
...
function ArtistHeader({
  ...
  const avatarLetter =
    (artist.username ?? artist.name)[0]?.toUpperCase() ?? "A";
  ...
  const customLinks = theme.links
    ...
    .map((link) => ({
      ...
      image:
        link.imagePreview ??
        (link.image_path ? storageUrl(link.image_path) : null),
    }));
  const socialLinks = [
  ...
  const handleHeaderMove = (event: globalThis.PointerEvent) => {
    ...
    if (drag.kind === "cover-frame") {
      const x = clamp(drag.startPositionX + rawDx, -320, 320);
    ...
    if (drag.kind === "avatar-border") {
      const x = clamp(drag.startPositionX + rawDx / 2, -80, 80);
      const y = clamp(drag.startPositionY + rawDy / 2, -80, 80);
      const tabsConfig = {
        ...theme.tabsConfig,
        border_offset: { x, y },
      };
    ...
    if (drag.kind === "cover-image") {
      const x = snapPercentCenter(
        clamp(drag.startPositionX - rawDx / 3, 0, 100),
      );
  ...
  return (
    <header className="relative border-b">
            ...
            {editMode && (
              <>
                <div className="absolute bottom-4 left-4 rounded-md bg-background/90 px-3 py-1 text-xs text-foreground shadow-sm">
                  Left-drag frame. Right-drag image. Use the white/blue corner
                  to resize.
                </div>
                <button
                  type="button"
      ...
      <div
        className="pointer-events-none absolute inset-x-0 top-0 mx-auto max-w-[1360px]"
        style={{ height: headerVisualHeight }}
      >
        <div
          className="pointer-events-auto absolute h-28 w-28 -translate-x-1/2 -translate-y-1/2"
          style={{
        ...
        >
          {selectedBorder && (
            <img
              ...
              alt=""
              className="pointer-events-none absolute -inset-4 z-0 h-[calc(100%+2rem)] w-[calc(100%+2rem)] object-contain"
              style={{
                transform: `translate(${borderOffset.x}px, ${borderOffset.y}px)`,
              }}
              draggable={false}
            />
          )}
          {editMode && selectedBorder && (
            <button
              type="button"
              className="absolute -right-7 -top-7 z-20 rounded bg-background p-1 text-[10px] shadow-sm ring-1 ring-sky-400"
              onPointerDown={(event) =>
          ...
          <div
            className={`relative z-10 h-full w-full overflow-hidden bg-primary text-primary-foreground flex items-center justify-center text-3xl font-bold ${
      ...
      <div className="max-w-[1360px] mx-auto px-4 pb-6 pt-16 text-center">
        <h1 className="mt-3 text-2xl font-bold">{artist.name}</h1>
        <p className="text-sm text-muted-foreground">@{artist.username}</p>
        {draft.artistTitle && (
          <p className="mt-1 text-sm">{draft.artistTitle}</p>
        )}
        {links.length > 0 && (
...
function ManageProfileSidebar({
  ...
  const canvasButtons = getCanvasItems(
    draft.tabsConfig,
    PROFILE_TAB_IDS,
    "tab",
  );
  const canvasSections = getCanvasItems(
    draft.tabsConfig,
    PROFILE_TAB_IDS,
    "section",
  );
  const visiblePreviewTabs = PROFILE_TAB_IDS.filter(
    (tab) => draft.tabsConfig.visibility[tab],
  );
      ...
      <div className="hidden gap-5 md:grid">
        <div className="grid gap-3">
          ...
          <p className="rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
            Left-drag the profile image to move the parent. Right-drag the image
            to reposition its content. Drag near the center to snap.
          </p>
        </div>
        ...
        <div className="grid gap-3">
          ...
          <form onSubmit={handleCreateBorder} className="grid gap-2">
            ...
            <Input
              ...
              onChange={(event) =>
                setBorderFile(event.target.files?.[0] ?? null)
              }
            />
        ...
        <div className="grid gap-3">
          ...
          <label className="flex items-center gap-2 text-sm">
            <input
              ...
              onChange={(event) =>
                onChange({ hasGradient: event.target.checked })
              }
            />
            Gradient
          </label>
        ...
        <div className="grid gap-3">
          ...
          />
          <p className="rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
            Together keeps tab buttons grouped. Separate lets individual tab
            buttons move on the canvas. Content widgets are added to the active
            preview page.
          </p>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          onClick={() => onRemoveCanvasItem(item.id, item.kind)}
                          aria-label={`Delete ${PROFILE_TAB_LABELS[item.type]}`}
                        >
                      ...
                      {item.kind === "section" && (
                        <div className="grid gap-2 border-t pt-2">
                          <SelectField
                            ...
                            formatOption={(option) =>
                              PROFILE_TAB_LABELS[option as ProfileTabId]
                            }
                            onChange={(page) =>
                              onUpdateCanvasItem(item.id, item.kind, {
      ...
      {block.type === "text" ? (
        <div className="mt-3 grid gap-2">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              size="icon-sm"
              onClick={() =>
                patch({ font_size: clamp(block.font_size - 2, 10, 96) })
              }
            >
        ...
        <div className="mt-3 grid gap-2">
          <ImageSourceControls
            ...
            onSelect={(value) =>
              value && setSource({ source_art_image_id: value })
            }
          />
          <SelectField
            label="Fit"
  ...
  return (
    <div
      ...
      onPointerDown={(event) => {
        ...
        if ((event.target as HTMLElement).closest("[data-board-control]"))
          return;
        onSelect(block.id);
      ...
      {editMode && (
        <>
          <div
            ...
            onPointerDown={(event) =>
              onBeginDrag(event, block, "padding-x", "left")
            }
          />
          <div
            data-board-control
            ...
            onPointerDown={(event) =>
              onBeginDrag(event, block, "padding-x", "right")
            }
          />
          <div
            data-board-control
            ...
            onPointerDown={(event) =>
              onBeginDrag(event, block, "padding-y", "bottom")
            }
          />
          <button
            data-board-control
...
function ArtsMasonry({
  ...
  return (
    <div
      ...
      style={{ columnWidth: display === "pinterest" ? Math.max(160, tileWidth - 30) : tileWidth }}
    >
      {images.map(({ art, image }, index) => (
        <button
          type="button"
            ...
            {works.map((work) => (
              <tr key={work.id} className="border-t">
                <td className="px-3 py-2">
                  <Link to={`/works/${work.slug}`} className="font-medium hover:underline">
                    {work.title}
                  </Link>
                </td>
...
function ProfileComments({
  ...
  if (comments.length === 0) {
    return (
      <EmptyPanel
        icon={MessageCircle}
        text="No public comments highlighted yet"
      />
    );
  }
            ...
            {comments.map((comment) => (
              <tr key={comment.id} className="border-t align-top">
                <td className="px-3 py-2">
                  ...
                  {comment.origin.href ? (
                    <Link to={comment.origin.href} className="font-medium hover:underline">
...
function blockImageSrc(block: ArtistProfileBlock) {
  if (block.source_sticker?.image_path)
    return storageUrl(block.source_sticker.image_path);
  if (block.source_art_image?.image_path)
    return storageUrl(block.source_art_image.image_path);
...
function parseCanvasDropPayload(
  ...
  if (
    (kind !== "tab" && kind !== "section") ||
    !PROFILE_TAB_IDS.includes(type as ProfileTabId)
  ) {
    return null;
  }
...
function findNearestOpenRect(
  ...
  const maxY = Math.max(
    getBoardRows(blocks) + GRID_STEP * 16,
    preferred.y + GRID_STEP * 16,
  );
...
function getBoardHeight(
  ...
  return Math.max(
    minHeight,
    (getBoardRows(blocks) + GRID_STEP * 8) * BOARD_UNIT_PX,
  );
}
...
function createProfileThemeDraft(
  artist: ArtistProfileResponse["artist"],
): ProfileThemeDraft {
  return {
    ...
    gradientDirection:
      artist.profile_background_gradient_direction ?? "to bottom",
...
function profileThemeToFormData(draft: ProfileThemeDraft, header: HeaderDraft) {
  ...
  draft.links.forEach((link, index) => {
    if (link.imageFile)
      payload.append(`profile_link_images[${index}]`, link.imageFile);
  });
...
function validateProfileEdit(
  ...
  draft.links.forEach((link, index) => {
    ...
    } else if (!isValidProfileUrl(link.url)) {
      errors[`links.${index}.url`] =
...
function normalizeProfileTabsConfig(
  ...
  return {
    ...
    border_offset: {
      x: clamp(value.border_offset?.x ?? 0, -80, 80),
      y: clamp(value.border_offset?.y ?? 0, -80, 80),
    },
  };
}
...
function getVisibleProfileTabs(
  config: ProfileTabsConfig,
  isStorytellerProfile: boolean,
) {
  return PROFILE_TAB_IDS.filter((tab) => {
    if ((tab === "arts" || tab === "works") && !isStorytellerProfile)
      return false;
...
function getCanvasItems(
  ...
  return items.filter(
    (item) => item.kind === kind && visibleTabs.includes(item.type),
  );
}
...
function getRenderableProfileTabs(
  tabs: ProfileTabId[],
  config: ProfileTabsConfig,
) {
  ...
  if (tabs.includes("board")) {
    if (
      config.section_mode === "board_arts" ||
